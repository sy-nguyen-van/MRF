...
OPT.last_vtk_filename = filename;
...
% Write elemental stresses (if stress is a response)
if OPT.write_stress_to_vtk
    fprintf(fid, '\n');
    fprintf(fid, "stress 1 %i float\n", FE.n_elem);
    for iel=1:FE.n_elem
        stress = full(FE.svm(iel));
        fprintf(fid, '%f \n', stress);
    end
end
fclose(fid);
